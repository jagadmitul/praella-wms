...
export class ExportsService {
   ...
   */
  async streamProducts(orgContext: OrgContext, response: Response): Promise<void> {
    const scope = warehouseScopeFilter(orgContext);

    await this.stream(response, ['sku', 'name', 'category', 'supplier', 'unit', 'unitPrice', 'totalQuantity', 'totalReserved', 'reorderPoint', 'isActive'], async (skip) => {
      const rows = await this.prisma.product.findMany({
        ...
        include: {
          ...
          stockLevels: {
            ...
            select: { quantity: true, reservedQuantity: true, reorderPoint: true },
          },
        },
      });
      ...
      return rows.map((product) => [
        ...
        product.stockLevels.reduce((sum, level) => sum + level.reservedQuantity, 0),
  ...
  async streamStockLevels(
    ...
    await this.stream(response, ['sku', 'product', 'warehouseCode', 'warehouse', 'quantity', 'reserved', 'available', 'reorderPoint', 'reorderQuantity', 'belowThreshold'], async (skip) => {
  ...
  async streamMovements(
    ...
    await this.stream(response, ['occurredAt', 'type', 'sku', 'product', 'warehouseCode', 'counterpartCode', 'quantity', 'balanceAfter', 'unitCost', 'reference', 'note', 'recordedBy'], async (skip) => {
  ...
  private async stream(
    ...
    response.write(toCsv(headers.map((header, index) => ({
      header,
