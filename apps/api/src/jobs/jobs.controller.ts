...
export class JobsController {
  ...
  @ApiBody({
    ...
    schema: { type: 'string', example: 'sku,warehouseCode,delta,reason\nELEC-MOU-02,SRT-HUB,5,Cycle count' },
  })
  @ApiAcceptedResponse({ description: 'Job queued' })
  async enqueueBulkStockCsv(
    ...
    return this.jobsService.enqueueBulkStockAdjustment(orgContext, user.id, { lines });
  }
   ...
   */
  private static parseStockCsv(
    csv: string,
  ): Array<{ sku: string; warehouseCode: string; delta: number; reason?: string }> {
    const records = parseCsvRecords(csv);

    if (records.length === 0) {
      throw new BadRequestException('The CSV contains a header but no data rows');
    }
    ...
    return records.map((record, index) => {
      ...
      if (!sku) throw new BadRequestException(`Line ${line}: "sku" is required`);
      if (!warehouseCode) {
        throw new BadRequestException(`Line ${line}: "warehouseCode" is required`);
      }
